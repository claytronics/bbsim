...
void process_tap(int x, int y)
{
	...
	fprintf(stderr, "%u tapped", block->id);
	msg2vm(block, CMD_TAP, block->localTime);
	block->tapBuffer++;
}
...
void event_loop()
{
  //fprintf(stderr, "Before call to glut In Event Loop\n");
  glutMainLoop();
  fprintf(stderr, "After call to glut In Event Loop\n");
  sauverEcran("toto");
}
